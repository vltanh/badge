...
class ActiveLearningByLearning(BaseStrategy):
    def __init__(self, X, Y, idxs_lb, net, handler, args, strategy_list, delta=0.1):
        super(ActiveLearningByLearning, self).__init__(
    ...
    def query(self, n):
        if not self.start:
            ...
            P = self.predict(self.X[idxs_labeled],
                             self.Y.numpy()[idxs_labeled])
            fn = (P.numpy() == self.Y.numpy()[idxs_labeled]).astype(float)
            reward = (fn / self.aw[self.idxs_lb]).mean()

            self.w[self.s_idx] *= np.exp(self.pmin/2.0 * (reward + 1.0 / self.last_p * np.sqrt(
                np.log(self.n_strategy / self.delta) / self.n_strategy)))
        ...
        print('  select {}'.format(
            type(self.strategy_list[self.s_idx]).__name__))
